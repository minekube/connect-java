...
approval, and a real guest login evidenced by both a new `<guest> joined the
game` host-log line and a new `Loaded ... advancements` guest-log line. When
that name is hidden, the privacy-filtered activity response is the authority
and the raw status probe is intentionally skipped. Startup or control-plane
reachability alone does not pass.
