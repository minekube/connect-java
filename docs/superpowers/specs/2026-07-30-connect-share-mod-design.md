...
Kotlin domain and runtime code uses Arrow as its default functional toolkit.
Expected failures are typed with `Raise`/`Either`; independent validation errors
are accumulated; managed tunnel/channel/runtime lifetimes use Arrow resource
scopes; and Arrow Fx/Resilience/Optics/STM capabilities replace local
equivalents when their use case exists. Fabric, Minecraft, and Java Core
boundaries keep their native signatures and adapt into Arrow at the edge. The
scoped rules and exceptions live in `share/AGENTS.md`.
