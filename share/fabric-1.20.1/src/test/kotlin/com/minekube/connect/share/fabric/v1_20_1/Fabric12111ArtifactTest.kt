...
class Fabric1201ArtifactTest {
    @Test
    fun `artifact runs on the standard Minecraft 1201 Java runtime`() {
        JarFile(artifact().toFile()).use { jar ->
            val metadata = jar.getInputStream(
                jar.getJarEntry("fabric.mod.json"),
            ).bufferedReader().readText()
            assertTrue("\"java\": \">=17\"" in metadata)
            val mixins = jar.getInputStream(
                jar.getJarEntry(
                    "connect-share-fabric-1.20.1.mixins.json",
                ),
            ).bufferedReader().readText()
            assertTrue("\"compatibilityLevel\": \"JAVA_17\"" in mixins)

            listOf(
                "com/minekube/connect/share/ShareCoordinator.class",
                "com/minekube/connect/share/fabric/FabricShareBootstrap.class",
                "com/minekube/connect/share/fabric/v1_20_1/" +
                    "FabricConnectShare1201Client.class",
            ).forEach { name ->
                val bytes = jar.getInputStream(jar.getJarEntry(name)).readNBytes(8)
                val major =
                    (bytes[6].toInt() and 0xff) shl 8 or
                        (bytes[7].toInt() and 0xff)
                assertTrue(
                    major <= JAVA_17_CLASS_MAJOR,
                    "$name requires class version $major",
                )
            }
        }
    }
    ...
    fun `remapped artifact is self contained and isolates networking runtime`() {
        JarFile(artifact().toFile()).use { jar ->
            ...
            assertTrue("connect-share-fabric-1.20.1.mixins.json" in entries)
            assertTrue(
                "com/minekube/connect/share/fabric/v1_20_1/" +
    ...
    @Test
    fun `minecraft profile mapper uses the legacy mutable Mojang property map ABI`() {
        JarFile(artifact().toFile()).use { jar ->
            ...
            assertTrue("getProperties" in bytecode)
            assertTrue("com/mojang/authlib/properties/PropertyMap" in bytecode)
            assertFalse(
    ...
    private fun artifact(): Path {
        ...
        return Files.list(directory).use { paths ->
            paths.filter {
                it.name.startsWith("connect-share-fabric-1.20.1-") &&
    ...
    private companion object {
        const val JAVA_17_CLASS_MAJOR = 61
