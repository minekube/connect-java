...
base {
    archivesName = "connect-share-fabric-1.21.1"
}
...
dependencies {
    ...
    modImplementation("net.fabricmc.fabric-api:fabric-api:${Versions.fabricApi1211Version}")
...
val minecraftGameProfileFactory =
    "com/minekube/connect/share/fabric/v1_21_1/" +
...
val connectShareShadowJar = tasks.named<ShadowJar>("shadowJar") {
    ...
    archiveBaseName.set("connect-share-fabric-1.21.1")
...
val verifyConnectShareArtifactSize = tasks.register("verifyConnectShareArtifactSize") {
    ...
    doLast {
        ...
        logger.lifecycle("Connect Share Fabric 1.21.1 artifact: {} MiB", "%.1f".format(bytes / 1024.0 / 1024.0))
        check(bytes <= limit) { "Connect Share Fabric 1.21.1 exceeds the 63 MiB release budget ($bytes bytes)" }
    }
}
