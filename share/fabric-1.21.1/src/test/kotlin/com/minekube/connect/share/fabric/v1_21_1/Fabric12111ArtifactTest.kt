...
class Fabric1211ArtifactTest {
    ...
    fun `remapped artifact is self contained and isolates networking runtime`() {
        JarFile(artifact().toFile()).use { jar ->
            ...
            assertTrue("connect-share-fabric-1.21.1.mixins.json" in entries)
            assertTrue(
                "com/minekube/connect/share/fabric/v1_21_1/" +
    ...
    @Test
    fun `minecraft profile mapper preserves the mutable Mojang Guava ABI`() {
        JarFile(artifact().toFile()).use { jar ->
    ...
    private fun artifact(): Path {
        ...
        return Files.list(directory).use { paths ->
            paths.filter {
                it.name.startsWith("connect-share-fabric-1.21.1-") &&
